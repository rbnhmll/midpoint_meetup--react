      ...
      {results.map( (result) => {
        return(
          <div className="venueContainer" key={result.venue.id}>
            <img className="venue__image" src={result.venue.featuredPhotos.items[0].prefix + '300x300' + result.venue.photos.groups[0].items[0].suffix} />
            ...
            <a className="venue__link" href={`https://foursquare.com/v/${result.venue.id}`} target="_blank"><i className="fa fa-foursquare"></i> Visit On Foursquare</a>
          </div>
        )
      } )}
    </div>
